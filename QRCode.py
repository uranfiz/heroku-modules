...
class QRCodeMod(loader.Module):
    """Генерация QR-кода и распознавание QR"""
    ...
    strings = {
        ...
        "recognized_multi": "<b>распознано:</b>\n{}",
        "recognized_item": "<code>{}</code>\n",
        "download_error": "⚠️ Не удалось скачать фото: <code>{}</code>",
        "decode_error": "⚠️ Ошибка распознавания: <code>{}</code>",
        "no_pyzbar": "⚠️ <code>pyzbar</code> не установлен — распознавание недоступно.",
    }
    ...
    async def qrscan(self, message):
        ...
        if not self._pyzbar_ok:
            await utils.answer(message, self.strings("no_pyzbar"))
            return

        status = await utils.answer(message, self.strings("recognizing"))
        ...
        if len(found) == 1:
            text = self.strings("recognized").format(safe_text(found[0]))
        else:
            body = ""
            for item in found:
                body += self.strings("recognized_item").format(
                    safe_text(item)
                )
            text = self.strings("recognized_multi").format(body)
